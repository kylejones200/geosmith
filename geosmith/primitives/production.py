...
Migrated from geosuite.forecasting.production_analysis.
Layer 2: Primitives - Pure data operations.
"""
...
def analyze_temporal_coverage(
    ...
    Args:
        ...
        date_col: Name of date column, default 'ReportDate'.
    ...
    Example:
        ...
        >>>
        >>> df = pd.DataFrame({
        ...     'ReportDate': pd.date_range('2020-01-01', periods=100, freq='M'),
        ...     'Oil': np.random.rand(100) * 1000
        ... })
        >>> coverage = analyze_temporal_coverage(df, date_col='ReportDate')
    ...
    if well_id_col and well_id_col in df.columns:
        monthly_active = df.groupby(dates.dt.to_period("M"))[well_id_col].nunique()
...
def analyze_spatial_distribution(
    ...
    Args:
        ...
        lon_col: Name of longitude column, default 'Long'.
    ...
    Example:
        >>> from geosmith.primitives.production import analyze_spatial_distribution
        >>> import pandas as pd
        >>> import numpy as np
        >>>
        >>> df = pd.DataFrame({
        ...     'Lat': np.random.uniform(30, 35, 100),
        ...     'Long': np.random.uniform(-100, -95, 100),
        ...     'Oil': np.random.rand(100) * 1000
        ... })
        >>> spatial = analyze_spatial_distribution(df, production_col='Oil')
...
def aggregate_by_field(
    ...
    Example:
        >>> from geosmith.primitives.production import aggregate_by_field
        >>> import pandas as pd
        >>> import numpy as np
        >>>
        >>> df = pd.DataFrame({
        ...     'FieldName': ['Field_A', 'Field_A', 'Field_B'] * 10,
        ...     'Oil': np.random.rand(30) * 1000,
        ...     'Gas': np.random.rand(30) * 500
        ... })
        >>> field_agg = aggregate_by_field(df, production_cols=['Oil', 'Gas'])
        >>> print(f"Fields: {field_agg.index.tolist()}")
    """
...
def aggregate_by_pool(
    ...
    Args:
        df: DataFrame with production data.
        pool_col: Name of pool column, default 'Pool'.
    ...
    Example:
        ...
        >>>
        >>> df = pd.DataFrame({
        ...     'Pool': ['Pool_A', 'Pool_B'] * 10,
        ...     'Oil': np.random.rand(20) * 1000
        ... })
        >>> pool_agg = aggregate_by_pool(df, production_cols=['Oil'])
    """
    return aggregate_by_field(
        df, field_col=pool_col, production_cols=production_cols, date_col=date_col
    )
...
def aggregate_by_county(
    ...
    Args:
        df: DataFrame with production data.
        county_col: Name of county column, default 'County'.
        production_cols: List of production columns to aggregate.
    ...
    Example:
        ...
        >>>
        >>> df = pd.DataFrame({
        ...     'County': ['County_A', 'County_B'] * 10,
        ...     'Oil': np.random.rand(20) * 1000
        ... })
        >>> county_agg = aggregate_by_county(df, production_cols=['Oil'])
    """
    return aggregate_by_field(
        df,
        field_col=county_col,
...
def calculate_well_statistics(
    ...
    Args:
        df: DataFrame with production data.
        well_id_col: Name of well identifier column, default 'API_WELLNO'.
    ...
    Example:
        >>> from geosmith.primitives.production import calculate_well_statistics
        >>> import pandas as pd
        >>>
        >>> df = pd.DataFrame({
        ...     'API_WELLNO': ['Well_1', 'Well_1', 'Well_2'] * 10,
        ...     'Oil': np.random.rand(30) * 1000,
        ...     'ReportDate': pd.date_range('2020-01-01', periods=30, freq='M')
        ... })
        >>> well_stats = calculate_well_statistics(
        ...     df, production_cols=['Oil'], date_col='ReportDate'
        ... )
        >>> print(f"Wells analyzed: {len(well_stats)}")
    """
...
def calculate_production_summary(
    df: pd.DataFrame, production_cols: Optional[List[str]] = None
    ...
    Example:
        >>> from geosmith.primitives.production import calculate_production_summary
        >>> import pandas as pd
        >>> import numpy as np
        >>>
        >>> df = pd.DataFrame({
        ...     'Oil': np.random.rand(100) * 1000,
        ...     'Gas': np.random.rand(100) * 500
        ... })
        >>> summary = calculate_production_summary(df, production_cols=['Oil', 'Gas'])
    ...
    for col in available_cols:
        ...
        summary[col] = {
            ...
            "non_zero_pct": (
                float(100 * non_zero / len(data)) if len(data) > 0 else 0.0
            ),
            "count": len(data),
        }
...
def calculate_production_density(
    ...
    Args:
        ...
        lat_col: Name of latitude column, default 'Lat'.
        lon_col: Name of longitude column, default 'Long'.
        production_col: Name of production column, default 'Oil'.
    ...
    Example:
        >>> from geosmith.primitives.production import calculate_production_density
        >>> import pandas as pd
        >>> import numpy as np
        >>>
        >>> df = pd.DataFrame({
        ...
        ...     'Long': np.random.uniform(-100, -95, 100),
        ...     'Oil': np.random.rand(100) * 1000,
        ...     'API_WELLNO': ['Well_1'] * 50 + ['Well_2'] * 50
        ... })
    ...
    # Assign bins
    df_clean["lat_bin"] = pd.cut(
        df_clean[lat_col], bins=lat_bins, labels=False, include_lowest=True
    )
    df_clean["lon_bin"] = pd.cut(
        df_clean[lon_col], bins=lon_bins, labels=False, include_lowest=True
    )
    ...
    density_df["lat_center"] = (
        lat_bins[density_df["lat_bin"].astype(int)] + bin_size_degrees / 2
    )
    density_df["lon_center"] = (
        lon_bins[density_df["lon_bin"].astype(int)] + bin_size_degrees / 2
    )
...
def identify_production_hotspots(
    ...
    Args:
        ...
        well_id_col: Optional well identifier column.
        bin_size_degrees: Size of spatial bins in degrees, default 0.1.
        percentile_threshold: Percentile threshold for hotspot definition, default 90.0.

    Returns:
        DataFrame with hotspot bins (top percentile by production density):
            - All columns from calculate_production_density
            - 'percentile_threshold': Threshold percentile used
            - 'threshold_value': Density threshold value
            - 'is_hotspot': True for all rows (all are hotspots)

    Example:
        >>> from geosmith.primitives.production import identify_production_hotspots
        >>> import pandas as pd
        >>>
        >>> df = pd.DataFrame({
        ...     'Lat': np.random.uniform(30, 35, 100),
        ...
        ...     df, production_col='Oil', percentile_threshold=95.0
        ... )
    ...
    logger.info(
        f"Identified {len(hotspots)} hotspots "
        f"(top {100 - percentile_threshold:.1f}%)"
    )
